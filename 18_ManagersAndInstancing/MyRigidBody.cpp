...
Simplex::MyRigidBody::MyRigidBody(MyMesh* a_pMesh)
{
	...
	// Fill out the min and max for each vert
	...
	// Find the center by averaging the the min and max values
	// Generate a cube using the center coordinates that we just calculated
...
void Simplex::MyRigidBody::Render(MyCamera* a_pCamera, matrix4 a_m4Transform)
{
	...
	// Calculate and generate the sphere
	float* pTemp = new float[16];
	matrix4 m4Resize = glm::scale(a_m4Transform, vector3(fRadius));
	memcpy(pTemp, glm::value_ptr(a_m4Transform), 16 * sizeof(float)); // memcpy takes an amount of memory dictated by the last argument and pasts it somehwere else
	m_pBS->RenderWire(a_pCamera->GetProjectionMatrix(), a_pCamera->GetViewMatrix(), pTemp, 1);


	// Calculate and generate the cube
